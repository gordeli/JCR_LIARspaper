...
# text = "The app was poorly designed. It should have contained video or images to guide users. Also, the font choice and color was distracting. The inclusion of math problems and word problems was also completely unnecessary. And not all of the pages were right side up. I have used it 5 times."
# print('Wordforms: ', wordformtion(text))
#
# print('Wordlemmas: ', wordlemmas(text))
...
conn = sqlite3.connect('Liars7_pos20210129.sqlite') # The copy of the original database to use for iterating
...
shutil.copyfile('Liars7_pos20210129.sqlite', 'Liars7_w.sqlite')
conn_w = sqlite3.connect('Liars7_w.sqlite') # The database to be updated
cur_w = conn_w.cursor()

try:
    cur_w.execute('''ALTER TABLE Reviews ADD Content_words INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Content_words' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD Lemmatizable_Content_words INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Lemmatizable_Content_words' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD Words_from_Instructions INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Instructions_words' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD Semantic_recall_words INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Semantic_recall_words' exists already''')
    pass # handle the error
...
# text = "boring,simple,useless,un-needed,stupid,dumb,bad,bland,sad,indifferent,90,,,,,,,,,"
# text.split(',')
# words =text.split(',')
# words = list(filter(None, words))
# # print(words)
...
# for w in words:
#     # print(w)
#     semantic_stems.append(porter.stem(w))
# print(semantic_stems)

# print(BWK_depth('stiff'))
# print(BWK_depth('abrak'))

sqlstr = 'SELECT Participants.id, Condition, Review_cleaned, Word_recall FROM Participants JOIN Reviews JOIN [Word Recall] ON Participants.id = Reviews.id = [Word Recall].id' # Select query that instructs over what we will be iterating
for row in cur.execute(sqlstr):
    ...
    reviewtext = row[2]
    semantic_text = row[3]
    semantic_words = semantic_text.split(',')
    semantic_words = list(filter(None, semantic_words))
    semantic_stems = []
    for w in semantic_words:
        semantic_stems.append(porter.stem(w))
    wordforms = wordformtion(reviewtext)
    content_words = 0
    semantic_recall_words = 0
    for w in wordforms:
        if w[1][0] in ['N', 'V', 'J', 'R'] or w[1] == 'CD':
            content_words += wordforms[w]
            if porter.stem(w[0]) in semantic_stems:
                semantic_recall_words += wordforms[w]
            else: continue
        else: continue
    review_lemmas = wordlemmas(reviewtext)
    lemmatizable_content_words = 0
    words_from_instructions = 0
    for l in review_lemmas:
        lemmatizable_content_words += review_lemmas[l]
        if l in excl:
            words_from_instructions += review_lemmas[l]


    cur_w.execute('UPDATE Reviews SET Content_words = ? WHERE id = ?', (content_words, id, ))
    cur_w.execute('UPDATE Reviews SET Lemmatizable_Content_words = ? WHERE id = ?', (lemmatizable_content_words, id, ))
    cur_w.execute('UPDATE Reviews SET Words_from_Instructions = ? WHERE id = ?', (words_from_instructions, id, ))
    cur_w.execute('UPDATE Reviews SET Semantic_recall_words = ? WHERE id = ?', (semantic_recall_words, id, ))
...
shutil.copyfile('Liars7_w.sqlite', 'Liars7_istr_recall20210129.sqlite')
