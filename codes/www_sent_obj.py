...
shutil.copyfile('Liars7_LIWC_sent_fixed.sqlite', 'Liars7_w.sqlite') # we need to create an extra database to use it to generate another search query, because we will need a nested loop (a loop with a subloop)
conn = sqlite3.connect('Liars7_LIWC_sent_fixed.sqlite')
cur = conn.cursor()
...
try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD What INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'What' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_obj INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'WWW_ST_obj' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_pairs_obj INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'WWW_ST_pairs_obj' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_emb_obj INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'WWW_ST_emb_obj' exists already")
    pass # handle the error
...
sqlstr = 'SELECT id, WC_LIWC, space_LIWC, time_LIWC, percept_LIWC, cause_LIWC, Sentence FROM Sentences_raw'
for row in cur.execute(sqlstr):
    ...
    parsed_sentense = nlp(sentence)
    what = 0 # initiate the direct objects counter
    for text in parsed_sentense:
        if text.dep_ == "dobj":
            what += 1

    www_ST_emb_obj = min(what, space_emb, time_emb)
    cur_w.execute('UPDATE Sentences_raw SET What = ?, WWW_ST_emb_obj = ? WHERE id = ?', (what, www_ST_emb_obj, prmr_id, ))

    if space_count > 0 and time_count > 0 and what > 0:
        www_ST_obj = 1
        www_STpairs_obj = min(what, space_count, time_count)
        cur_w.execute('UPDATE Sentences_raw SET WWW_ST_obj = ?, WWW_ST_pairs_obj = ? WHERE id = ?', (www_ST_obj, www_STpairs_obj, prmr_id, ))
...
shutil.copyfile('Liars7_w.sqlite', 'Liars7_www_sent_obj.sqlite')
shutil.copyfile('Liars7_www_sent_obj.sqlite', r"C:\Users\i_gordeliy\Dropbox\Marketing\Liars\Liars7git\Liars7_www_sent_obj.sqlite")
