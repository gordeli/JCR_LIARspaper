...
conn = sqlite3.connect('Liars7_www_new.sqlite')
...
shutil.copyfile('Liars7_www_new.sqlite', 'Liars7_w.sqlite')
conn_w = sqlite3.connect('Liars7_w.sqlite')
cur_w = conn_w.cursor()

# Create columns for LIWC data for our fitered dictionary:

try: # cleaned
    cur_w.execute('''ALTER TABLE Reviews ADD space_rel_LIWC REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'space_rel_LIWC' exists already''')
    pass # handle the error

try: # cleaned
    cur_w.execute('''ALTER TABLE Reviews ADD time_rel_LIWC REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
...
name = "LIWC_Reviews_TimeSpaceRel.xls"
...
for r in range(1, sheet.nrows):
    if sheet.cell_type(r, 0) == (xlrd.XL_CELL_EMPTY, xlrd.XL_CELL_BLANK): continue
        ...
        try: # attempt to extract the content of the relevant cells
            ...
            cur_w.execute('''UPDATE Reviews SET (space_rel_LIWC, time_rel_LIWC) = (?, ?) WHERE id = ?''', (spaceRel, timeRel, id))
...
cur.close()
conn.close()
shutil.copyfile('Liars7_w.sqlite', 'Liars7_LIWC_rel.sqlite')
