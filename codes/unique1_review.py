...
import sqlite3
import shutil
conn = sqlite3.connect('Liars7_cwf_diversity.sqlite')
# Get the cursor, which is used to traverse the database, line by line
cur = conn.cursor()
shutil.copyfile('Liars7_cwf_diversity.sqlite', 'Am_kg_w.sqlite')
conn_w = sqlite3.connect('Am_kg_w.sqlite') # The database to be updated
cur_w = conn_w.cursor()

try:
    cur_w.execute('''ALTER TABLE Reviews ADD Review_uniqwords1 INTEGER NOT NULL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print("The column 'Review_uniqwords1' exists already")
...
for row in cur.execute(sqlstr):
    wordform = row[0]
    TF = row[1]
    if TF < 2: # This number is highly corpus specific (53.1 for kaggle21k, 27 for Liars, 27.39 for MAm_kg?), 6.3 for Liars Pilot, 20.16 for Liars 7 (reviews only), 54.58 for yelp_CHIhotels
...
for row in cur.execute(sqlstr):
    ...
    cur_w.execute('UPDATE Reviews SET Review_uniqwords1 = ? WHERE Review_cleaned = ?', (unique_words,reviewtext, ))
...
shutil.copyfile('Am_kg_w.sqlite', 'Liars7_unique1.sqlite')
