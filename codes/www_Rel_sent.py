...
shutil.copyfile('Liars7_LIWC_Rel_sent.sqlite', 'Liars7_w.sqlite') # we need to create an extra database to use it to generate another search query, because we will need a nested loop (a loop with a subloop)
conn = sqlite3.connect('Liars7_LIWC_Rel_sent.sqlite')
cur = conn.cursor()
...
try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_Rel INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'WWW_ST_Rel' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_Rel_pairs INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'WWW_ST_Rel_pairs' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD Space_Rel_emb INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'Space_Rel_emb' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD Time_Rel_emb INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
    print("The column 'Time_Rel_emb' exists already")
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Sentences_raw ADD WWW_ST_Rel_emb INTEGER NOT NULL DEFAULT 0''') # TF-er is the originally calculated value wich colculated the sum of numbers of unique nouns in each review  DEFAULT 0 was removed from the sql string
except:
...
sqlstr = 'SELECT id, WC_LIWC, space_Rel_LIWC, time_Rel_LIWC, percept_LIWC, cause_LIWC FROM Sentences_raw'
for row in cur.execute(sqlstr):
    ...
    cur_w.execute('UPDATE Sentences_raw SET Space_Rel_emb = ?, Time_Rel_emb = ?, WWW_ST_Rel_emb = ? WHERE id = ?', (space_emb, time_emb, www_ST_emb, prmr_id, ))

    if space_count > 0 and time_count > 0:
        www_ST = 1
        www_STpairs = min(space_count, time_count)
        cur_w.execute('UPDATE Sentences_raw SET WWW_ST_Rel = ?, WWW_ST_Rel_pairs = ? WHERE id = ?', (www_ST, www_STpairs, prmr_id, ))
...
shutil.copyfile('Liars7_w.sqlite', 'Liars7_www_Rel_sent.sqlite')
