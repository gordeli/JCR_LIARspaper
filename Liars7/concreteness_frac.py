...
def concrete_words(text, postag, repetitions, exclusion_list):
    # text is is the input uncleaned text for which to calculate depth
    # postag - pos tag included in the count
    # repetitions - true (include repetitions in the counts), false ( count only once) what to do if the same wordform is repeated
    # output is the total concreteness and the number of words used in the calculation
    ...
    # 20200526: 2 options input text or input the all_nouns object, which is a dictionary that relates the key (worform, pos) to the value (noun it transforms into)
    # from scipy.special import comb #scipy is a mathematical packet and comb is the function we need to calculate the entropy
    number_concrete_nouns = 0 # initiating the number of concrete nouns
                ...
                if type(noun) == list:
                    ...
                    for n in noun:
                        ...
                        if (len(n)<2 or (depth_n == 0 and n != 'entity')): continue # make sure the words which are not in WordNet and/or give depth 0 are not included. usually those are incorrectly assigned parts of speech. there was 298 of them without this line of code
                            ...
                            frequency = word_forms[wordform] #all_nouns is a dictionary in which the keys are the nouns and their values are their frequencies
                            number = number * (depth_n + 1) # Note depth_noun needs to be bigger by 1, but 1 is being subtracted
                    if number > 6:
                        number_concrete_nouns += frequency
                    ...
                    try:
                        ...
                        print(noun, wordform[1], nouns)
                        break
                    if (len(noun) < 2 or (depth_noun == 0 and noun != 'entity')): continue # make sure the words which are not in WordNet and/or give depth 0 are not included. usually those are incorrectly assigned parts of speech. there was 298 of them without this line of code
                    else:
                        frequency = word_forms[wordform] #all_nouns is a dictionary in which the keys are the nouns and their values are their frequencies
                        number_nouns = number_nouns + frequency #number_nouns is the number of nouns in the review
                        # number = comb(depth_noun + 1 + frequency - 1, frequency) # Note depth_noun needs to be bigger by 1, but 1 is being subtracted
                        if depth_noun + 1 > 6:
                            number_concrete_nouns += frequency
    else:
        # Need to create a new nouns subdictionary, which only contains the proper POS tags
    ...
    return number_concrete_nouns, number_nouns
...
conn = sqlite3.connect('Liars7_BWK.sqlite') # The copy of the original database to use for iterating
# # Get the cursor, which is used to traverse the database, line by line
cur = conn.cursor()
...
shutil.copyfile('Liars7_BWK.sqlite', 'Liars7_w.sqlite')
conn_w = sqlite3.connect('Liars7_w.sqlite') # The database to be updated
cur_w = conn_w.cursor()

try:
    cur_w.execute('''ALTER TABLE Reviews ADD N_concrete_rep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_concrete_rep_excl' exists already''')
...
try:
    cur_w.execute('''ALTER TABLE Reviews ADD V_concrete_rep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_concrete_rep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD J_concrete_rep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_concrete_rep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD R_concrete_rep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_concrete_rep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD N_concrete_norep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_concrete_norep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD V_concrete_norep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_concrete_norep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD J_concrete_norep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_concrete_norep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD R_concrete_norep_excl INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_concrete_norep_excl' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD N_concrete_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_concrete_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD V_concrete_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_concrete_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD J_concrete_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_concrete_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD R_concrete_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_concrete_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD N_concrete_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_concrete_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD V_concrete_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_concrete_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD J_concrete_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_concrete_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE Reviews ADD R_concrete_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_concrete_norep' exists already''')
    pass # handle the error
...
for row in cur.execute(sqlstr):
    ...
    reviewtext = row[2]
    N_concrete_rep_excl = concrete_words(reviewtext, ['NN', 'CD'], True, excl)
    N_concrete_norep_excl = concrete_words(reviewtext, ['NN', 'CD'], False, excl)
    N_concrete_rep = concrete_words(reviewtext, ['NN', 'CD'], True, [])
    N_concrete_norep = concrete_words(reviewtext, ['NN', 'CD'], False, [])

    V_concrete_rep_excl = concrete_words(reviewtext, 'VB', True, excl)
    V_concrete_norep_excl = concrete_words(reviewtext, 'VB', False, excl)
    V_concrete_rep = concrete_words(reviewtext, 'VB', True, [])
    V_concrete_norep = concrete_words(reviewtext, 'VB', False, [])

    J_concrete_rep_excl = concrete_words(reviewtext, 'JJ', True, excl)
    J_concrete_norep_excl = concrete_words(reviewtext, 'JJ', False, excl)
    J_concrete_rep = concrete_words(reviewtext, 'JJ', True, [])
    J_concrete_norep = concrete_words(reviewtext, 'JJ', False, [])

    R_concrete_rep_excl = concrete_words(reviewtext, 'RB', True, excl)
    R_concrete_norep_excl = concrete_words(reviewtext, 'RB', False, excl)
    R_concrete_rep = concrete_words(reviewtext, 'RB', True, [])
    R_concrete_norep = concrete_words(reviewtext, 'RB', False, [])

    cur_w.execute('UPDATE Reviews SET N_concrete_rep_excl = ? WHERE id = ?', (N_concrete_rep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET V_concrete_rep_excl = ? WHERE id = ?', (V_concrete_rep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET J_concrete_rep_excl = ? WHERE id = ?', (J_concrete_rep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET R_concrete_rep_excl = ? WHERE id = ?', (R_concrete_rep_excl[0], id, ))

    cur_w.execute('UPDATE Reviews SET N_concrete_norep_excl = ? WHERE id = ?', (N_concrete_norep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET V_concrete_norep_excl = ? WHERE id = ?', (V_concrete_norep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET J_concrete_norep_excl = ? WHERE id = ?', (J_concrete_norep_excl[0], id, ))
    cur_w.execute('UPDATE Reviews SET R_concrete_norep_excl = ? WHERE id = ?', (R_concrete_norep_excl[0], id, ))

    cur_w.execute('UPDATE Reviews SET N_concrete_rep = ? WHERE id = ?', (N_concrete_rep[0], id, ))
    cur_w.execute('UPDATE Reviews SET V_concrete_rep = ? WHERE id = ?', (V_concrete_rep[0], id, ))
    cur_w.execute('UPDATE Reviews SET J_concrete_rep = ? WHERE id = ?', (J_concrete_rep[0], id, ))
    cur_w.execute('UPDATE Reviews SET R_concrete_rep = ? WHERE id = ?', (R_concrete_rep[0], id, ))

    cur_w.execute('UPDATE Reviews SET N_concrete_norep = ? WHERE id = ?', (N_concrete_norep[0], id, ))
    cur_w.execute('UPDATE Reviews SET V_concrete_norep = ? WHERE id = ?', (V_concrete_norep[0], id, ))
    cur_w.execute('UPDATE Reviews SET J_concrete_norep = ? WHERE id = ?', (J_concrete_norep[0], id, ))
    cur_w.execute('UPDATE Reviews SET R_concrete_norep = ? WHERE id = ?', (R_concrete_norep[0], id, ))
...
shutil.copyfile('Liars7_w.sqlite', 'Liars7_frac.sqlite')
