...
conn = sqlite3.connect('yelpCHIhotels_nouns.sqlite') # The copy of the original database to use for iterating
...
shutil.copyfile('yelpCHIhotels_nouns.sqlite', 'Liars7_w.sqlite')
conn_w = sqlite3.connect('Liars7_w.sqlite') # The database to be updated
cur_w = conn_w.cursor()

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Word_count INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
...
try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_N_rep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_N_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD N_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_N_norep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_N_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD N_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'N_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_V_rep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_V_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD V_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_V_norep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_V_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD V_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'V_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_J_rep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_J_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD J_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_J_norep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_J_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD J_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'J_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_R_rep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_R_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD R_rep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_rep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD Depth_R_norep REAL DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'Depth_R_norep' exists already''')
    pass # handle the error

try:
    cur_w.execute('''ALTER TABLE yelp_chi_hotels ADD R_norep INTEGER DEFAULT 0''') # DEFAULT 0 was removed from the sql string
except:
    print('''The column 'R_norep' exists already''')
    pass # handle the error

sqlstr = 'SELECT id, filtered, Review_cleaned FROM yelp_chi_hotels' # Select query that instructs over what we will be iterating
for row in cur.execute(sqlstr):
    id = row[0]
    condition = int((row[1]+1)/2)
    reviewtext = row[2]
    ...
    cur_w.execute('UPDATE yelp_chi_hotels SET Word_count = ? WHERE id = ?', (word_count, id, ))

    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_N_rep = ? WHERE id = ?', (review_depth_N_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET N_rep = ? WHERE id = ?', (N_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_N_norep = ? WHERE id = ?', (review_depth_N_norep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET N_norep = ? WHERE id = ?', (N_norep,id, ))

    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_V_rep = ? WHERE id = ?', (review_depth_V_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET V_rep = ? WHERE id = ?', (V_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_V_norep = ? WHERE id = ?', (review_depth_V_norep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET V_norep = ? WHERE id = ?', (V_norep,id, ))

    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_J_rep = ? WHERE id = ?', (review_depth_J_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET J_rep = ? WHERE id = ?', (J_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_J_norep = ? WHERE id = ?', (review_depth_J_norep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET J_norep = ? WHERE id = ?', (J_norep,id, ))

    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_R_rep = ? WHERE id = ?', (review_depth_R_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET R_rep = ? WHERE id = ?', (R_rep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET Depth_R_norep = ? WHERE id = ?', (review_depth_R_norep,id, ))
    cur_w.execute('UPDATE yelp_chi_hotels SET R_norep = ? WHERE id = ?', (R_norep,id, ))
...
shutil.copyfile('Liars7_w.sqlite', 'yelpCHIhotels_hypnum.sqlite')
